...
*   **Real-Time LinkedIn Monitoring:** Utilizes LinkUp webhooks to track target companies' LinkedIn activity, automatically collecting reactions and comments from posts. It includes smart scheduling (business hours only) and configurable collection settings per company to optimize credit usage. Leads collected are attributed to their source.
...
Key tables include `users`, `teams`, `prospect_candidates`, `leads`, `linkedinConnections` (LinkUp auth), `linkedin_oauth_credentials` (OAuth tokens), `target_companies`, `decision_makers`, `icp_profiles`, and tables for LinkedIn post automation and real-time monitoring (`webhook_accounts`, `monitored_companies`, `company_posts`, `lead_collection_configs`, `scheduled_collections`).
...
LinkUp API calls for profile enrichment are cached to prevent redundant usage. Real-time monitoring optimizes credit consumption through smart scheduling.

## External Dependencies

*   **Payment Processing:** Stripe (for subscriptions, billing, Checkout, Customer Portal, webhooks).
*   **Third-Party APIs:**
    *   **LinkUp API:** For LinkedIn data extraction and real-time monitoring.
